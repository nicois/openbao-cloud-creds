	...
	return []*framework.Path{
		{
			...
			Fields: map[string]*framework.FieldSchema{
				...
				fieldMintersKey: {Type: framework.TypeSlice, Description: "Minter credentials (access_key_id + secret_access_key) in this set"},
			},
...
func parseMinter(mMap map[string]interface{}) (cloudconfig.Minter, error) {
	...
	if err := cloudconfig.ValidateMinterKeys(fmt.Sprintf("%v", mMap["id"]), mMap,
		"id", "expires_at", neverExpiresKey, "access_key_id", "secret_access_key", cloudconfig.MinterKeyRotationParams); err != nil {
		return cloudconfig.Minter{}, err
	}
	accessKeyID := fmt.Sprintf("%v", mMap["access_key_id"])
	secretAccessKey := fmt.Sprintf("%v", mMap["secret_access_key"])
...
func (b *backend) pathMinterSetRotate(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	if oldIdx == -1 {
		return credenvelope.ErrorResponse(credenvelope.ErrConfigInvalid, "minter %q not found in set %q", minterID, name), nil
	}
	old := set.Minters[oldIdx]
	if old.Retired {
		return credenvelope.ErrorResponse(credenvelope.ErrConfigInvalid, "minter %q is already retired", minterID), nil
	}
