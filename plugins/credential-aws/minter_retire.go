package credentialaws

import (
	"context"
	"time"

	"github.com/nicois/openbao-cloud-creds/pkg/cloudconfig"
	"github.com/openbao/openbao/sdk/v2/logical"
)

// sweepRetiredMinters deletes the upstream access key of every minter that has
// been retired longer than minter_retire_grace, then drops it from its set. It
// is keyed strictly off RetiredAt (separate from the conservative orphan
// reconciler) so an operator-initiated rotation reliably reclaims the old IAM
// access key after the grace window — long enough that every raft node has
// reloaded the set and stopped selecting the retired minter.
//
// Reclaiming the slot also re-enables rotation: while a retired key lingers the
// IAM user holds 2 keys, so a fresh rotation would hit the 2-key make-before-
// break guard; the sweep frees one slot once the grace elapses.
//
// now is a parameter for testability. The whole sweep runs under rotateSweepMu
// (mirroring the rotate endpoint) so it never interleaves with a rotation's
// read-modify-write of a set; b.mu is taken only inside the small helpers
// (persistSet, buildIAMMinterClient), never across the body while rotateSweepMu
// is held, so no AB-BA deadlock with b.mu exists.
//
// access-key-id-for-retire: DeleteAccessKey needs the OLD key's upstream id.
// Rotation successors record their access key id in
// RotationParams[access_key_id], so once a successor is itself retired it can be
// deleted precisely. An operator-provided ORIGINAL minter may have no recorded
// access key id; for those the sweep cannot delete the upstream key precisely —
// it logs a warning and still drops the minter from the set after the grace (so
// issuance is never affected), leaving the upstream key for manual cleanup.
func (b *backend) sweepRetiredMinters(ctx context.Context, storage logical.Storage, now time.Time) error {
	b.rotateSweepMu.Lock()
	defer b.rotateSweepMu.Unlock()

	grace := b.retireGrace()

	names, err := storage.List(ctx, "minter-sets/")
	if err != nil {
		return err
	}
	for _, name := range names {
		if err := b.sweepSet(ctx, storage, name, now, grace); err != nil {
			b.Logger().Warn("retired-sweep: set failed; continuing",
				"cloud", cloudName, "minter_set", name, "error", err)
		}
	}
	return nil
}

// retireGrace returns the configured retirement grace, defaulting to the
// minimum minter gap when config is unset.
func (b *backend) retireGrace() time.Duration {
	b.mu.RLock()
	defer b.mu.RUnlock()
	if b.config != nil && b.config.MinterRetireGrace > 0 {
		return b.config.MinterRetireGrace
	}
	return cloudconfig.MinMinterGap
}

// sweepSet processes one set: it removes the upstream access key and drops every
// minter whose retirement has aged past the grace, then persists if anything
// changed.
func (b *backend) sweepSet(ctx context.Context, storage logical.Storage, name string, now time.Time, grace time.Duration) error {
	set, err := b.readSet(ctx, storage, name)
	if err != nil {
		return err
	}
	if set == nil {
		return nil
	}

	kept := make([]cloudconfig.Minter, 0, len(set.Minters))
	changed := false
	for i := range set.Minters {
		m := set.Minters[i]
		if m.Retired && now.After(m.RetiredAt.Add(grace)) {
			if b.deleteRetiredUpstream(ctx, m) {
				changed = true
				continue // drop from the set
			}
			// DeleteAccessKey failed (non-NoSuchEntity): keep the entry and retry.
		}
		kept = append(kept, m)
	}

	if !changed {
		return nil
	}
	set.Minters = kept
	return b.persistSet(ctx, storage, set)
}

// deleteRetiredUpstream removes a retired minter's upstream access key. It
// returns true when the entry may be dropped from the set: the key was deleted,
// was already gone (NoSuchEntity), or cannot be deleted precisely (no recorded
// access key id — warn and drop anyway so issuance isn't affected). It returns
// false only on a real DeleteAccessKey failure, so the entry is kept for retry.
func (b *backend) deleteRetiredUpstream(ctx context.Context, m cloudconfig.Minter) bool {
	accessKeyID := m.RotationParams[fieldAccessKeyID]
	if accessKeyID == "" {
		b.Logger().Warn("retired-sweep: minter has no recorded upstream access key id; "+
			"dropping from set after grace, upstream key needs manual cleanup",
			"cloud", cloudName, "minter_id", m.ID)
		return true
	}

	b.mu.RLock()
	client := b.buildIAMMinterClient(m)
	b.mu.RUnlock()
	if err := client.DeleteAccessKey(ctx, accessKeyID); err != nil && !isNoSuchEntity(err) {
		b.Logger().Warn("retired-sweep: upstream DeleteAccessKey failed; will retry next pass",
			"cloud", cloudName, "minter_id", m.ID, "error", err)
		return false
	}
	b.Logger().Info("retired-sweep: deleted upstream access key of retired minter",
		"cloud", cloudName, "minter_id", m.ID)
	return true
}
