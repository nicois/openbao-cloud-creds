	...
	return []*framework.Path{
		{
			...
			Fields: map[string]*framework.FieldSchema{
				"minters": {
					...
					Description: "List of minter credentials (access_key_id + secret_access_key)",
				},
				"region": {
...
func (b *backend) pathConfigWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	for _, m := range mintersSlice {
		...
		minter := cloudconfig.Minter{
			...
			Token:     fmt.Sprintf("%v", mMap["secret_access_key"]),
			CreatedAt: time.Now(),
		}
		// Store access_key_id in ExpiresSource field (overloading for convenience)
		// Actually, we need access_key_id stored somewhere. We'll put it in the token
		// field as "access_key_id:secret_access_key" joined format.
		accessKeyID := fmt.Sprintf("%v", mMap["access_key_id"])
		minter.Token = accessKeyID + ":" + fmt.Sprintf("%v", mMap["secret_access_key"])
