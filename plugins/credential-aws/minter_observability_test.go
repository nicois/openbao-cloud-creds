...
// newObservabilityBackend builds an AWS backend via Factory with a buffer-backed
// logger (so warn-logs are assertable) and the given minter-set written through the
// API. AWS is an injected-client plugin with no HTTP fake: the minter-set write path
// does not call STS, so minters load into b.minterSets directly and emitMinterMetrics
// can run without a live endpoint. minterExpiryWarnSecs is the config threshold in
// seconds (0 = omit, use default).
func newObservabilityBackend(t *testing.T, minters []interface{}, minterExpiryWarnSecs int) (*backend, *strings.Builder, *gometrics.InmemSink, logical.Storage) {
	...
	cfgData := map[string]interface{}{configRegionKey: "us-east-1"}
...
func TestEmitMinterMetrics_AgeGaugeForNeverExpires(t *testing.T) {
	bk, logBuf, sink, _ := newObservabilityBackend(t, []interface{}{
		map[string]interface{}{"id": "m1", minterAccessKeyIDKey: "AKIA1", minterSecretAccessKeyKey: "secret1", "never_expires": true},
	}, 0)
...
func TestEmitMinterMetrics_WarnsWithinThreshold(t *testing.T) {
	...
	bk, logBuf, _, _ := newObservabilityBackend(t, []interface{}{
		map[string]interface{}{"id": "m1", minterAccessKeyIDKey: "AKIA1", minterSecretAccessKeyKey: "secret1", fieldExpiresAt: soon},
		map[string]interface{}{"id": "m2", minterAccessKeyIDKey: "AKIA2", minterSecretAccessKeyKey: "secret2", fieldExpiresAt: later},
	}, 0)
...
func TestEmitMinterMetrics_NoWarnOutsideThreshold(t *testing.T) {
	...
	bk, logBuf, _, _ := newObservabilityBackend(t, []interface{}{
		map[string]interface{}{"id": "m1", minterAccessKeyIDKey: "AKIA1", minterSecretAccessKeyKey: "secret1", fieldExpiresAt: a},
		map[string]interface{}{"id": "m2", minterAccessKeyIDKey: "AKIA2", minterSecretAccessKeyKey: "secret2", fieldExpiresAt: bExpiry},
	}, 0)
