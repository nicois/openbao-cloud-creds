...
func (b *backend) emitMinterMetrics() {
	...
	for setName, states := range b.minterSets {
		for id, ms := range states {
			...
			labels = withCloudKey(labels, cloudconfig.CloudKeyID(ms.minter, fieldTokenID))
			...
			if !ms.minter.ExpiresAt.IsZero() {
				...
				if expiresIn > 0 && expiresIn < warnThreshold {
					b.Logger().Warn("minter nearing expiry",
						"cloud", cloudName,
						"cloud_key_id", cloudconfig.CloudKeyID(ms.minter, fieldTokenID), "minter_set", setName, "minter_id", id,
