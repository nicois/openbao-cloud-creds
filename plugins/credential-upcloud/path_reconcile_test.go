...
const upcloudTokenPrefix = "cloud-creds-"
...
func TestPathReconcile_FloorsSubMinHold(t *testing.T) {
	...
	srv.AddRawTokenWithCreatedAt("orphan-1", upcloudTokenPrefix+"role-x", time.Now().Add(-1*time.Minute).UTC().Format(time.RFC3339))
...
func TestPathReconcile_DeletesOldOrphanAboveFloor(t *testing.T) {
	...
	b, storage := setupConfiguredBackend(t, srv.URL)
	srv.AddRawTokenWithCreatedAt("orphan-1", upcloudTokenPrefix+"role-x", time.Now().Add(-10*time.Minute).UTC().Format(time.RFC3339))
