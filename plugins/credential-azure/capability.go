...
func (b *backend) capabilityChecks(set *cloudconfig.MinterSet, roleJSON []byte) []capability.Check {
	...
	return capability.ChecksPerMinter(set, role.Name, role.AppObjectID, func(minter cloudconfig.Minter) func(context.Context) (int, error) {
...
func (b *backend) probeMint(minter cloudconfig.Minter, roleName, appObjectID string) func(context.Context) (int, error) {
	return func(ctx context.Context) (int, error) {
		...
		pw, status, err := client.AddPassword(ctx, appObjectID,
			capability.ProbeName(ownertag.Prefix(b.ownerInstance()), roleName), time.Now().Add(probePasswordLifetime))
		if err != nil {
			return status, fmt.Errorf("probe addPassword on app %s returned %d: %w", appObjectID, status, err)
		}
