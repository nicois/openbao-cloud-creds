...
func TestSelectMinter_SkipsCooldownMinter(t *testing.T) {
	...
	req := &logical.Request{
		...
		Data: map[string]any{"tenant_id": "test-tenant-id"},
	}
	...
	req = &logical.Request{
		...
		Data: map[string]any{
			"minters": []any{
				map[string]any{"id": "minter-1", minterTokenKey: "client-id-1:secret-1", "never_expires": true},
				map[string]any{"id": "minter-2", minterTokenKey: "client-id-2:secret-2", "never_expires": true},
			},
		},
	}
	if resp, err := b.HandleRequest(t.Context(), req); err != nil || (resp != nil && resp.IsError()) {
