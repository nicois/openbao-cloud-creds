...
// active-tokens/ tracking record cannot be persisted, the plugin revokes the
// just-minted upstream password credential and returns an error envelope
// rather than leaving a live-but-untracked credential (audit F6).
func TestCredsIssue_TrackingWriteFailureCompensates(t *testing.T) {
	...
	if got := srv.ProvisionedCount(); got != before {
		t.Fatalf("orphaned upstream credential: count went %d -> %d (live-but-untracked credential, F6)", before, got)
	}
}
