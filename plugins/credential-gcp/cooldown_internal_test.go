...
func TestSelectMinter_SkipsCooldownMinter(t *testing.T) {
	...
	req = &logical.Request{
		...
		Data: map[string]interface{}{
			"minters": []interface{}{
				map[string]interface{}{"id": "minter-1", minterCredentialsJSONKey: "{\"k\":1}", "never_expires": true},
				map[string]interface{}{"id": "minter-2", minterCredentialsJSONKey: "{\"k\":2}", "never_expires": true},
			},
		},
	}
	if resp, err := b.HandleRequest(t.Context(), req); err != nil || (resp != nil && resp.IsError()) {
