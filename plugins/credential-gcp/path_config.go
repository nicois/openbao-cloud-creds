	...
	return []*framework.Path{
		{
			...
			Fields: map[string]*framework.FieldSchema{
				"minters": {
					...
					Description: "List of minter credentials (service account JSON key files)",
				},
				"project": {
...
func (b *backend) pathConfigWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	for _, m := range mintersSlice {
		...
		// Store the credentials JSON in the Token field
		if credJSON, ok := mMap["credentials_json"].(string); ok {
			minter.Token = credJSON
		} else {
			return logical.ErrorResponse("minter %s: credentials_json is required", minter.ID), nil
		}
