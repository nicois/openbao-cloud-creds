	...
	return []*framework.Path{
		{
			...
			Fields: map[string]*framework.FieldSchema{
				...
				fieldMintersKey: {Type: framework.TypeSlice, Description: "Minter credentials (client_id/client_secret pairs) in this set"},
			},
...
// For OVH, each minter carries a client_id and client_secret, which we store
// colon-separated in the Token field; the token client constructor splits it.
...
func parseMinter(mMap map[string]interface{}) (cloudconfig.Minter, error) {
	...
	if err := cloudconfig.ValidateMinterKeys(fmt.Sprintf("%v", mMap["id"]), mMap,
		"id", "expires_at", neverExpiresKey, "client_id", "client_secret"); err != nil {
	...
	if clientID == "" || clientSecret == "" {
		return cloudconfig.Minter{}, fmt.Errorf("minter %s: client_id and client_secret are required", minter.ID)
	}
