...
// purgeDeleter deletes one of a role's issued API clients upstream.
//
// The minter is chosen the way a lease revoke chooses one: the issuing minter if it is still
// in the set, else any healthy minter in it. Either can delete the API client — it belongs to
// the account, not to the credential that created it — and the issuing minter may well be the
// one that has just been rotated out because it leaked.
func (b *backend) purgeDeleter(role *akamaiRole) upstreampurge.Deleter {
	return func(ctx context.Context, rec upstreampurge.Record) error {
		...
		now := time.Now()
		status, err := client.DeleteClient(ctx, rec.ID)
