...
func testEdgeGridCred() *edgeGridCredential {
	return &edgeGridCredential{
		ClientToken:  "akab-ct-test",
		AccessToken:  "akab-at-test",
		ClientSecret: "akab-cs-test",
	}
}

// TestReconcile_PopulatesCreatedAt proves the lister surfaces the API client's
// creation timestamp (Akamai's "createdDate" field) into UpstreamEntity.CreatedAt
// (audit F1 precision layer).
...
func TestReconcile_CreatedAtDrivesDeleteVsSkip(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			deleted := !srv.HasClient("orphan-1")
