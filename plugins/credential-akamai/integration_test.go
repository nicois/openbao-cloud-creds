...
func TestFullLifecycle(t *testing.T) {
	...
	// Verify envelope
	if issueResp.Data["cloud"] != "akamai" {
	...
	if meta["minter_set"] != "default" || meta["minter_id"] != "minter-1" {
		t.Fatalf("bad provenance metadata: %v", issueResp.Data["metadata"])
	}
	if meta["issued_by"] != "cloud-creds-akamai/v0.1" {
		t.Fatalf("bad issued_by: %v", meta["issued_by"])
	}

	// Verify credential contains all Akamai fields
	cred, ok := issueResp.Data["credential"].(map[string]interface{})
	if !ok {
		t.Fatalf("expected credential map, got %T", issueResp.Data["credential"])
	}
	if cred["client_token"] == nil || cred["client_token"] == "" {
		t.Fatal("expected non-empty credential.client_token")
	}
	if cred["access_token"] == nil || cred["access_token"] == "" {
		t.Fatal("expected non-empty credential.access_token")
	}
	if cred["client_secret"] == nil || cred["client_secret"] == "" {
		t.Fatal("expected non-empty credential.client_secret")
	}
	if cred["host"] == nil || cred["host"] == "" {
		t.Fatal("expected non-empty credential.host")
	}
