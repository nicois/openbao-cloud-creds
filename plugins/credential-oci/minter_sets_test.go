...
func TestMinterSetCRUD(t *testing.T) {
	...
	req := &logical.Request{
		...
		Data: map[string]any{
			"minters": []any{
				map[string]any{"id": "m1", "token": "tenancy:user:fingerprint:key", "never_expires": true},
			},
		},
	}
...
func TestMinterSetValidationRejectsSingleExpiring(t *testing.T) {
	...
	req := &logical.Request{
		...
		Data: map[string]any{
			"minters": []any{
				map[string]any{"id": "m1", "token": "tenancy:user:fingerprint:key", "expires_at": "2027-01-01T00:00:00Z"},
			},
		},
	}
	resp, err := b.HandleRequest(t.Context(), req)
