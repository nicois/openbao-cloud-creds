...
// TestMinterSetRotate_RejectsAndDoesNotMutate verifies the uniform minter rotate
// endpoint exists but rejects with a clear "not supported" message and leaves
// the minter set entirely unchanged. OCI uses phased slot rotation and cannot
// self-rotate minter identities; the minter-set rotate endpoint is reject-only.
func TestMinterSetRotate_RejectsAndDoesNotMutate(t *testing.T) {
	...
	if resp, err := b.HandleRequest(t.Context(), &logical.Request{
		...
		Data: map[string]any{mintersKey: []any{
...
func TestConfigMinterRetireGraceRejectedWhereNothingRetires(t *testing.T) {
	...
	resp, err := b.HandleRequest(t.Context(), &logical.Request{
		Operation: logical.UpdateOperation, Path: pathConfigKey, Storage: storage,
		Data: map[string]any{testRegionField: testRegionValue, fieldMinterRetireGrace: graceSeconds},
	})
	if err != nil {
