	...
	return []*framework.Path{
		{
			...
			Fields: map[string]*framework.FieldSchema{
				...
				fieldMintersKey: {Type: framework.TypeSlice, Description: "Minter credentials in this set. Each entry: {id, token (tenancy_ocid:user_ocid:fingerprint:private_key_pem), never_expires/expires_at}"},
			},
...
// parseMinters converts the raw fieldMintersKey field into cloudconfig.Minters. The
// OCI minter token ("tenancy_ocid:user_ocid:fingerprint:private_key_pem") is
// read as the opaque Token string unchanged.
...
func parseMinter(mMap map[string]any) (cloudconfig.Minter, error) {
	...
	if err := cloudconfig.ValidateMinterKeys(fmt.Sprintf("%v", mMap["id"]), mMap,
		"id", "expires_at", neverExpiresKey, "token"); err != nil {
