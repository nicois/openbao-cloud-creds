...
func TestCredsRead_OverdueSlotNotServed(t *testing.T) {
	...
	for i := 0; i < role.SlotCount; i++ {
		s, err := loadSlot(context.Background(), storage, "test-role", i)
		if err != nil || s == nil {
			continue
		}
		s.NextRotationAt = time.Now().Add(-time.Hour)
		if err := saveSlot(context.Background(), storage, "test-role", s); err != nil {
...
func TestCredsRead_FreshSlotServedWhenAnotherOverdue(t *testing.T) {
	...
	s1, err := loadSlot(context.Background(), storage, "test-role", 1)
	if err != nil || s1 == nil {
	...
	if err := saveSlot(context.Background(), storage, "test-role", s1); err != nil {
