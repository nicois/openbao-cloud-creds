...
func newObservabilityBackend(t *testing.T, minters []any, minterExpiryWarnSecs int) (*backend, *strings.Builder, *gometrics.InmemSink, logical.Storage) {
	...
	cfgData := map[string]any{testRegionField: testRegionValue}
	...
	if resp, err := b.HandleRequest(t.Context(), &logical.Request{
		...
		Data: map[string]any{mintersKey: minters},
	}); err != nil || (resp != nil && resp.IsError()) {
...
func TestEmitMinterMetrics_AgeGaugeForNeverExpires(t *testing.T) {
	bk, logBuf, sink, _ := newObservabilityBackend(t, []any{
		map[string]any{"id": "m1", minterTokenKey: "tenancy:user1:fp:key", "never_expires": true},
	}, 0)
...
func TestEmitMinterMetrics_WarnsWithinThreshold(t *testing.T) {
	...
	bk, logBuf, _, _ := newObservabilityBackend(t, []any{
		map[string]any{"id": "m1", minterTokenKey: "tenancy:user1:fp:key", fieldExpiresAt: soon},
		map[string]any{"id": "m2", minterTokenKey: "tenancy:user2:fp:key", fieldExpiresAt: later},
	}, 0)
...
func TestEmitMinterMetrics_NoWarnOutsideThreshold(t *testing.T) {
	...
	bk, logBuf, _, _ := newObservabilityBackend(t, []any{
		map[string]any{"id": "m1", minterTokenKey: "tenancy:user1:fp:key", fieldExpiresAt: a},
		map[string]any{"id": "m2", minterTokenKey: "tenancy:user2:fp:key", fieldExpiresAt: bExpiry},
	}, 0)
