...
// newObservabilityBackend builds an OCI backend via Factory with a buffer-backed
// logger (so warn-logs are assertable) and the given minter-set written through the
// API. OCI is phased-rotation with an injected client (no HTTP fake): a fake OCI
// client is injected via SetClientFactory and the minter-set write path loads minters
// into b.minterSets directly, so emitMinterMetrics can run without a live endpoint.
...
func newObservabilityBackend(t *testing.T, minters []interface{}, minterExpiryWarnSecs int) (*backend, *strings.Builder, *gometrics.InmemSink, logical.Storage) {
	...
	bk := b.(*backend)
	// Route per-set minter clients through an in-memory fake so the rotation
	// worker (spawned on config write) does not build a real OCI client.
	bk.SetClientFactory(func(string) OCIIAMClient { return NewTestFakeClient() })
	storage := config.StorageView

	cfgData := map[string]interface{}{testRegionField: testRegionValue}
	...
	if resp, err := b.HandleRequest(context.Background(), &logical.Request{
		...
		Data: map[string]interface{}{mintersKey: minters},
	}); err != nil || (resp != nil && resp.IsError()) {
...
func TestEmitMinterMetrics_AgeGaugeForNeverExpires(t *testing.T) {
	bk, logBuf, sink, _ := newObservabilityBackend(t, []interface{}{
		map[string]interface{}{"id": "m1", minterTokenKey: "tenancy:user1:fp:key", "never_expires": true},
	}, 0)
...
func TestEmitMinterMetrics_WarnsWithinThreshold(t *testing.T) {
	...
	bk, logBuf, _, _ := newObservabilityBackend(t, []interface{}{
		map[string]interface{}{"id": "m1", minterTokenKey: "tenancy:user1:fp:key", fieldExpiresAt: soon},
		map[string]interface{}{"id": "m2", minterTokenKey: "tenancy:user2:fp:key", fieldExpiresAt: later},
	}, 0)
...
func TestEmitMinterMetrics_NoWarnOutsideThreshold(t *testing.T) {
	...
	bk, logBuf, _, _ := newObservabilityBackend(t, []interface{}{
		map[string]interface{}{"id": "m1", minterTokenKey: "tenancy:user1:fp:key", fieldExpiresAt: a},
		map[string]interface{}{"id": "m2", minterTokenKey: "tenancy:user2:fp:key", fieldExpiresAt: bExpiry},
	}, 0)
