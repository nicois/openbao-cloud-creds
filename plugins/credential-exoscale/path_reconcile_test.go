...
// boundary: an upstream API key whose name does not carry the cloud-creds-
...
// floor (surfaced in the response), and that because Exoscale's list API
// carries no creation timestamp (KI-004), a cloud-creds-prefixed orphan has a
// zero CreatedAt and is therefore skipped by the fail-closed reconciler
// whenever the hold is > 0. So a floored manual reconcile deletes nothing here.
