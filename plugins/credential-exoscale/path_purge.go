...
//
// It reads the role directly rather than through the issuance path's loader, which refuses a
// disabled role: disable-then-purge is the sequence this whole pair of levers is for. A role
// disabled to stop the bleeding must still be purgeable, or an operator would have to
// re-enable it — briefly reopening issuance — to destroy what leaked.
...
func (b *backend) purgeDeleter(role *exoscaleRole) upstreampurge.Deleter {
	return func(ctx context.Context, rec upstreampurge.Record) error {
		...
		now := time.Now()
		status, err := client.DeleteAPIKey(ctx, rec.ID)
