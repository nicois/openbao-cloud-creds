...
func TestEmitMinterMetrics_AgeGaugeForNeverExpires(t *testing.T) {
	bk, logBuf, sink, _ := newObservabilityBackend(t, []any{
		map[string]any{"id": "m1", minterTokenKey: "vultr_key_a", "never_expires": true},
	}, 0)
...
func TestEmitMinterMetrics_WarnsWithinThreshold(t *testing.T) {
	...
	bk, logBuf, _, _ := newObservabilityBackend(t, []any{
		map[string]any{"id": "m1", minterTokenKey: "vultr_key_a", fieldExpiresAt: soon},
		map[string]any{"id": "m2", minterTokenKey: "vultr_key_b", fieldExpiresAt: later},
	}, 0)
...
func TestEmitMinterMetrics_NoWarnOutsideThreshold(t *testing.T) {
	...
	bk, logBuf, _, _ := newObservabilityBackend(t, []any{
		map[string]any{"id": "m1", minterTokenKey: "vultr_key_a", fieldExpiresAt: a},
		map[string]any{"id": "m2", minterTokenKey: "vultr_key_b", fieldExpiresAt: bExpiry},
	}, 0)
