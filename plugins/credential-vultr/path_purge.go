...
// purgeDeleter deletes one of a role's issued credentials upstream — here a sub-user, which is
// what this cloud issues a credential as.
//
// The minter is chosen the way a lease revoke chooses one: the issuing minter if it is still
// in the set, else any healthy minter in it. Either can delete the sub-user — it belongs to
...
func (b *backend) purgeDeleter(role *vultrRole) upstreampurge.Deleter {
	return func(ctx context.Context, rec upstreampurge.Record) error {
		...
		now := time.Now()
		status, err := client.DeleteUser(ctx, rec.ID)
