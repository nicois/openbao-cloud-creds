...
func newResilienceHarness(t *testing.T) (plugintest.Harness, *fakes.VultrServer) {
	...
	configure := func(t *testing.T, b logical.Backend, storage logical.Storage) {
		...
		write("config", map[string]interface{}{"vultr_api_url": srv.URL})
	...
	rewriteWithout := func(t *testing.T, b logical.Backend, storage logical.Storage) {
		resp, err := b.HandleRequest(context.Background(), &logical.Request{
			...
			Data: map[string]interface{}{
				"minters": []interface{}{
					map[string]interface{}{"id": "minter-2", "token": "vultr_reseeded_key", "never_expires": true},
				},
			},
		})
		if err != nil || (resp != nil && resp.IsError()) {
