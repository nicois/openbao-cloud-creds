...
| :---- | :---- | :---- |
| `cloud-creds/<cloud>/config` | write | Configure plugin: operational + cloud settings (`flush_interval`, `reconcile_cadence`, `minter_expiry_warn`, `minter_retire_grace`, region/endpoints). **Minters are NOT configured here** — see minter-sets. |
...
| `cloud-creds/<cloud>/minter-sets/<name>/rotate` | write | Operator-initiated minter self-rotation (`minter_id=<id>`). Mints a mint-capable successor, health-checks it, swaps it in, and marks the old minter retired (deleted by the sweep after `minter_retire_grace`). Implemented for Azure/UpCloud/AWS/Akamai/GCP/Exoscale; DO/OVH/Vultr/OCI reject (API cannot mint a mint-capable successor). |
