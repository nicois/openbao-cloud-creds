...
| `cloud-creds/<cloud>/roles/<name>` | write/read/delete/list | CRUD on credential roles. Each role has a **required** `minter_set` field naming the set it mints from, a `disabled` flag (settable since 2026-09-16 on all ten plugins) that stops issuance without deleting anything, and `require_caller_identity` (`none` default / `any` / `token_accessor`, added 2026-09-20) which refuses to issue to a caller core cannot name — `token_accessor` refuses a batch token, whose entity belongs to the service token that created it. Refusals carry `caller_unidentified`. Also `require_caller_lineage` (`none` default / `parent` / `live_parent`, added 2026-09-21) which refuses a caller whose parent this mount cannot establish, or whose parent entity is absent or disabled; refusals carry `caller_unparented`. |
...
| `cloud-creds/<cloud>/issued` | list | Inventory of credentials this mount has issued and NOT yet revoked: key = upstream credential id, `key_info` = role, minter, timestamps and who obtained it (`requested_by_token_accessor`/`requested_by_entity_id`). Fields are published by allowlist, so no credential material can reach it. Pages with `after`/`limit`. Refused with `unsupported` on OCI, which keeps no per-credential record (a read serves a shared slot) — an empty success would read as "nothing is outstanding". An inventory, not an audit log: an entry disappears when its credential is revoked. |
...
| API-003 | List operations over CONFIGURATION (`roles`, `minter-sets`) return arrays without pagination: those counts are bounded by what an operator wrote, in the hundreds. **`issued/` is the exception and pages** (`after`/`limit`, cursor opaque, `more` stated rather than inferred): it is the one keyspace whose size is the fleet's, one record per outstanding credential, so an unbounded listing is a response nobody can receive. Revised 2026-09-20; the original "no pagination at this stage" predates that endpoint. |
...
| :---- | :---- |
| **Request** | No body. Two OPTIONAL query parameters, both request-only — neither appears in the response envelope, so neither affected `api_version`. `credential_kind=<kind>` pins the shape of the `credential` block the caller can parse; a mismatch is refused with `credential_kind_unsupported` before anything is loaded or minted (see the envelope row). `shard_key=<opaque>` names the calling worker, selecting which minter of the role's set issues this credential: callers sharing a key share a minter, different keys are spread across the set by rendezvous hash. Omitting it falls back to the caller's token accessor, then its identity entity, then to no affinity at all — so it is genuinely optional and a client that never sends it still gets per-token spreading. Whether affinity also multiplies the ISSUED credential's rate limit is per-cloud: on six clouds the credential belongs to the minter's account, while on AWS/GCP/Azure its identity is a target named on the ROLE and is the same whichever minter issued it. Rationale and the per-cloud table: [`docs/decisions.md`](decisions.md). |
...
| **Additional Details** | Envelope shape: `{"cloud": str, "role": str, "credential": <cloud-specific>, "expires_at": iso8601, "ttl_seconds": int, "renewable": bool, "credential_id": str, "metadata": {"scope": str, "scope_kind": str, "credential_kind": str, "issued_by": str, "api_version": "4", "minter_set": str, "minter_id": str}}`. `metadata.minter_set` and `metadata.minter_id` identify the minting credential that issued this credential, for audit provenance (added in api_version 2). **`metadata.scope_kind` (added in api_version 3)** says how to read `metadata.scope`, from a closed vocabulary — `scopes` (a permission-scope list: DO, Akamai), `acl` (coarse categories: Vultr), `role` (an upstream role whose permissions the credential assumes: AWS's IAM role ARN, Exoscale's role id), `identity` (the principal whose credential this is: GCP service account, Azure app registration, OCI user), `account` (no per-credential scoping at all — `scope` is then EMPTY: UpCloud, OVH), `grants` (per-RESOURCE grants, each `<resource>:<permission>`, account-wide rendered `*` — DO Spaces buckets; distinct from `scopes` because a grant's left half names one bucket while a scope's names a resource *type*). Before v3, `scope` was one field name carrying all of those meanings with nothing to distinguish them, so a client had to hard-code an interpretation per cloud (A28). **`metadata.credential_kind` (added in api_version 4)** names the SHAPE of the `credential` block — `sigv4_session`, `oauth2_bearer`, `basic_auth`, `bearer_token`, `scoped_token`, `key_secret`, `azure_client_secret`, `edgegrid`, `oci_auth_token`, `s3_credentials` — and a caller may **pin** it by passing `credential_kind=<kind>` on a credential read. A mismatch is refused with `credential_kind_unsupported` before anything is loaded or minted, rather than returning a payload the caller cannot parse. This is what makes a NEW shape additive: a cloud can grow a second one (AWS SES over SMTP needs `{username, password}` derived from a static key, because SMTP has nowhere to put a session token) and an older client keeps getting the shape it asked for, or an actionable error. A kind names a payload rather than a cloud, so two clouds emitting identical keys share one — GCP and OVH are both `oauth2_bearer` — and that is enforced by a conformance test over the registry, not merely intended. The first cloud to serve TWO shapes is DigitalOcean (2026-09-15): a role's `credential_type` selects `scoped_token` or `s3_credentials`, so on that plugin the served kind is a function of the role rather than a constant — which is the case the pin was designed for. `s3_credentials` is `{access_key_id, secret_access_key, endpoint, region}` (+ `session_token` where a cloud grants one) and is named for the S3 protocol rather than a vendor, because several plugins are expected to serve it; `endpoint` and `region` are inside the credential block because an S3 client cannot be built without them and neither is derivable from the key material. **Per-cloud `credential` schemas — all ten, and asserted exactly by the conformance `lease` category** (`Harness.CredentialKeys`, so this table cannot drift from the code again): AWS `{access_key_id, secret_access_key, session_token}`; GCP `{access_token, token_type}`; Azure `{client_id, client_secret, tenant_id}` plus `subscription_id` when the role names one; DO `{token, scopes[]}` for a `credential_type=token` role and `{access_key_id, secret_access_key, endpoint, region}` for a `credential_type=spaces_key` one; UpCloud `{username, password}`; OVH `{access_token, token_type}`; Exoscale `{key, secret}` — **both halves; the secret is returned only at creation and was previously dropped**; Vultr `{api_key}`; Akamai `{client_token, access_token, client_secret, host}`; OCI `{auth_token, user_id}`. |
