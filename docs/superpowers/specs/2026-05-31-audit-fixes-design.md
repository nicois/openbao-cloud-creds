...
```
**RESOLVED mechanism (verified empirically during implementation):** OpenBao only surfaces `resp.Error()` — the `Data["error"]` message string — to clients on error responses; `Data` side-channels (including a nested `data` map) are dropped (`IsError()` requires Data to be exactly `{error}` or `{error,data}`, and the HTTP error path extracts only the message). Therefore the code MUST ride in the message string. The helper returns `logical.ErrorResponse("%s: %s", string(code), msg)` → a standard single-key `{"error": "<code>: <message>"}` response. Clients parse the `"<code>: "` prefix. This centralizes the previously ad-hoc `"role_not_found: ..."` prefixing behind the typed `ErrorCode` constants.

**Test contract:** assert via `resp.Error().Error()` (client-visible), NOT `resp.Data["error_code"]`. Pattern: require `resp.IsError()`, then `strings.HasPrefix(resp.Error().Error(), string(code)+": ")`.
